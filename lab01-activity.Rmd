...
```{bash, engine.opts='-l'}
head -n 25 /local-scratch/course_files/MBB243/Sneddon_genotypes.txt > cat ~/MBB243/genotype_head.txt 
# translation:
...
# the file doesn't exist, because if we used > then it'll create a new file. But since we used >> as well, which will append to an existing file (which doesn't exist).
```
...
```{bash, engine.opts='-l'}
$DATA="/local-scratch/course_~/files/MBB243/" 
